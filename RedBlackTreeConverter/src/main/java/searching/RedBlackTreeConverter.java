...
public class RedBlackTreeConverter {
    ...
    public static<Key extends Comparable<Key>> RBNode<Key> convert(TwoThreeNode<Key> twoThreeNode) {
        ...
        RBNode<Key> parentB = null;
        if(twoThreeNode.is3node()){
            parentB = new RBNode<>(twoThreeNode.rightKey,twoThreeNode.rightValue,Color.Black,0);
            RBNode<Key> parentR = new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Red, 0);
            parentB.leftChild = parentR;
            parentB.rightChild = convert(twoThreeNode.rightChild);
            parentR.rightChild = convert(twoThreeNode.centerChild);
            parentR.leftChild = convert(twoThreeNode.leftChild);
            parentR.size = (parentR.leftChild != null ? parentR.leftChild.size : 0)
                    + (parentR.rightChild != null ? parentR.rightChild.size : 0)
                    + 1;

            parentB.size =  parentR.size
                    + (parentB.rightChild != null ? parentB.rightChild.size : 0)
                    + 1;
        }

        if(twoThreeNode.is2node()){
            parentB = new RBNode<>(twoThreeNode.leftKey,twoThreeNode.leftValue,Color.Black,0);
            parentB.leftChild = convert(twoThreeNode.leftChild);
            parentB.rightChild = convert(twoThreeNode.centerChild);
            parentB.size = (parentB.leftChild != null ? parentB.leftChild.size : 0)
                    + (parentB.rightChild != null ? parentB.rightChild.size : 0)
                    + 1;
