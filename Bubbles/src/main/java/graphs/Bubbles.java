...
public class Bubbles {
    ...
    public static List<ForbiddenRelation> cleanBubbles(List<Contact> contacts, int n) {
        Map<String, HashSet<String>> bulles = new HashMap<>();
        List<ForbiddenRelation> forbidden = new ArrayList<>();

        for( Contact contact : contacts){
            if(!bulles.containsKey(contact.a)){
                HashSet<String> con = new HashSet<>();
                bulles.put(contact.a,con );
            }
            if(!bulles.containsKey(contact.b)){
                HashSet<String> con = new HashSet<>();
                bulles.put(contact.b,con );
            }
            HashSet<String> bulleA = bulles.get(contact.a);
            HashSet<String> bulleB = bulles.get(contact.b);
            bulleA.add(contact.b);
            bulleB.add(contact.a);
            bulles.put(contact.a, bulleA);
            bulles.put(contact.b, bulleB);
        }

        PriorityQueue<String> queue = new PriorityQueue<>(
                (p1, p2) -> Integer.compare(bulles.get(p2).size(), bulles.get(p1).size())
        );
        queue.addAll(bulles.keySet());

        while (!queue.isEmpty()) {


            String maxPerson = queue.poll();
            if (bulles.get(maxPerson).size() <= n) break;

            String toRemove = null;
            List<String> tempList = new ArrayList<>();
            tempList.add(maxPerson);
            while (!queue.isEmpty()) {
                String candidate = queue.poll();
                tempList.add(candidate);
                if (bulles.get(candidate).contains(maxPerson)) {
                    toRemove = candidate;
            ...
            if (toRemove != null) {
                bulles.get(maxPerson).remove(toRemove);
                bulles.get(toRemove).remove(maxPerson);
                forbidden.add(new ForbiddenRelation(maxPerson, toRemove));
            }
