...
public class Trains {
    ...
    public static Map<String, Integer> reachableEarliest(HashMap<StationTime, LinkedList<StationTime>> relations, StationTime startPoint) {
        HashMap<String,TreeMap<Integer, List<StationTime>>> stationtimes = new HashMap<>();
        HashMap<String,Integer> fastestStationTimes = new HashMap<>();
        PriorityQueue<StationTime> pq = new PriorityQueue<>(
                Comparator.comparingInt(st -> fastestStationTimes.get(st.station)));

        for(StationTime times: relations.keySet()){
            TreeMap<Integer,List<StationTime>> timestation;
            if (stationtimes.get(times.station) == null){
                timestation = new TreeMap<>();
                fastestStationTimes.put(times.station, Integer.MAX_VALUE);
            }else {
                timestation = stationtimes.get(times.station);
            }
            timestation.put(times.time, relations.get(times));
            stationtimes.put(times.station,timestation);
            for(StationTime sT : relations.get(times)){
                if (!fastestStationTimes.containsKey(sT.station)){
                    fastestStationTimes.put(sT.station,Integer.MAX_VALUE);
                }
            }
        }

        pq.add(startPoint);
        fastestStationTimes.put(startPoint.station, startPoint.time);
        HashSet<String> nonvisited ;
        while(!pq.isEmpty()){
             StationTime currStation = pq.poll();
             if(currStation.time > fastestStationTimes.get(currStation.station) || !stationtimes.containsKey(currStation.station)){
                 continue;
             }
             String currstat = currStation.station;


             NavigableMap<Integer, List<StationTime>> nextTrainsMap =
                    stationtimes.get(currstat).tailMap(fastestStationTimes.get(currstat), true);

            nonvisited = new HashSet<>(fastestStationTimes.keySet());
            nonvisited.remove(currstat);

             for (List<StationTime> trainList : nextTrainsMap.values()) {

                 for (StationTime nextTime : trainList) {
                    String nextStation = nextTime.station;

                        if(fastestStationTimes.get(nextStation) > nextTime.time){
                            fastestStationTimes.put(nextStation, nextTime.time);
                            pq.add(nextTime);
                        }
        ...
        HashSet<String> stats = new HashSet<>(fastestStationTimes.keySet());
        for(String stat : stats){
            if(fastestStationTimes.get(stat) == Integer.MAX_VALUE){
                fastestStationTimes.remove(stat);
            }
        }


        return fastestStationTimes;
    }
